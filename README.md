...
The starter is pinned to the immutable Linux/AMD64 image used in ProxyWar's
hosted Commander canary. It runs Commander Arm C: Claude chooses among a small
typed menu of strategic options, while deterministic code converts that choice
to an exact currently offered `LegalAction.id`. The model never emits a raw game
intent or executable action ID.
